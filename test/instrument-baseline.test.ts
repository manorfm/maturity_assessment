...
test('linha de base mede percurso, autoria e profundidade por perspectiva', () => {
  ...
  assert.equal(baseline.graphVersion, 'evidence-anamnesis-pilot-v13');
...
test('linha de base torna visível dívida de fundamentos e contratos de direção', () => {
  ...
  assert.equal(baseline.comparison.version, 'capability-reference-v10');
  assert.equal(baseline.comparison.references, 13);
  assert.equal(baseline.comparison.taxonomyCoverage, 13);
  assert.equal(baseline.comparison.coverageVersion, 'capability-reference-coverage-v1');
  assert.equal(baseline.comparison.coverage.length, 13);
  assert.deepEqual(
