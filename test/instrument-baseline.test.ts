...
test('linha de base mede percurso, autoria e profundidade por perspectiva', () => {
  ...
  assert.equal(baseline.graphVersion, 'evidence-anamnesis-pilot-v15');
...
test('linha de base torna visível dívida de fundamentos e contratos de direção', () => {
  ...
  assert.equal(baseline.comparison.version, 'capability-reference-v14');
  assert.equal(baseline.comparison.references, 17);
  assert.equal(baseline.comparison.taxonomyCoverage, 17);
  assert.equal(baseline.comparison.coverageVersion, 'capability-reference-coverage-v1');
  assert.equal(baseline.comparison.coverage.length, 17);
  assert.deepEqual(
