...
const projectForm = (error = '') => layout('Novo projeto', `
  ...
  ${error ? `<p class="notice warning">${escapeHtml(error)}</p>` : ''}
...
export function registerProjectRoutes(app: FastifyInstance, db: Database): void {
  ...
  app.post('/projects', async (request, reply) => {
    ...
    if (!body.name?.trim() || !body.hierarchy?.trim()) return reply.code(400).type('text/html').send(projectForm('Informe nome e ao menos uma unidade.'));
  ...
  app.get('/p/:publicId', async (request, reply) => {
    ...
    if (!project) return reply.code(404).send('Projeto não encontrado');
  ...
  const authorize = (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!project) { reply.code(404).type('text/html').send(layout('Acesso inválido', '<div class="card"><h1>Acesso inválido</h1><p>Confira o link administrativo.</p></div>')); return undefined; }
    return { params, project };
  };

  app.get('/projects/:publicId/manage/:adminSecret', async (request, reply) => {
    const auth = authorize(request, reply); if (!auth) return;
  ...
  app.post('/projects/:publicId/manage/:adminSecret/invitations', async (request, reply) => {
    ...
    if (!body.unitId || !units.some((unit) => unit.id === body.unitId) || !body.profile || !profiles[body.profile]) return reply.code(400).send('Configuração inválida');
