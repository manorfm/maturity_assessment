...
export function renderOutcome(outcome: ReportOutcome, context: { confirmedProblemCount?: number; occurrence?: FindingScopeOccurrence; competingFinding?: OutcomeFinding } = {}): string {
  ...
  const briefing = guidance && (outcome.kind === 'correct' || outcome.kind === 'evolve')
    ? `<section><h3>O que está acontecendo</h3><p class="executive-reading">${escapeHtml(guidance.plainExplanation)}</p><p>${escapeHtml(guidance.mechanism)}</p></section>${affected}${priority}${evidenceBlock}${diagnosticContext}${causalAnalysis}${readinessBlock}${metaSystem}
