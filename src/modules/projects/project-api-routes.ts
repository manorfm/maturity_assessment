...
function sanitizeReport(report: ReturnType<InferenceService['report']>) {
  const finding = (item: { capability: string; detailCapability: string; pattern: string; title: string; intervention: string }) => ({ capability: item.capability, detailCapability: item.detailCapability, pattern: item.pattern, title: item.title, intervention: item.intervention });
  return {
