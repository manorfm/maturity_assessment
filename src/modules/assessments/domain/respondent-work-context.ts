...
export class RespondentWorkContext implements WorkContextData {
  ...
  static fromJSON(raw: string): RespondentWorkContext | undefined {
    ...
    try {
      ...
      if (!validAuthority(value.authority) || !validScope(value.scope) || !Array.isArray(value.observableEvents) || !value.observableEvents.every((item) => validEvent(item))) throw new Error();
  ...
  toJSON(): WorkContextData {
    return { responsibilities: this.responsibilities, authority: this.authority, scope: this.scope, observableEvents: this.observableEvents };
  }
}
